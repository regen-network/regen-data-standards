...
2. **Does JC's engine already produce RDFC-1.0 canonical N-Quads over an equivalent substance set?** **Answered 2026-09-08 by reading the repository, and the two halves come apart: RDFC-1.0 yes, equivalent substance set no.**
   ...
   - **Substance: no, and it differs on *both* of his branches, differently.** On `main` the preimage is **Claim IRI + canonical content + assertor IRI + `asserted_at`** (`crates/claims/src/domain/fingerprint.rs`). On `claude/claims-engine-domain-model-8odk0u` (`217fafdd`, 2 Jul — a 553-line design doc plus a full L0/L1 implementation, **unmerged**) it is **declared schema references + canonical content**, with the ClaimIRI *derived from* the fingerprint rather than an input to it, and provenance removed from the claim value entirely (design doc §19). Note `main` is the merged default branch, so a casual reader gets the older model.
