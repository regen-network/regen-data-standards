...
- **Status:** Proposed — **revised 2026-08-06** (D2 rationale re-grounded on the v2 enum; D8 collapsed; JC-assumption claim withdrawn). D2 recommendation was reversed 2026-07-31. See the changelog at the foot. Needs ratification by Darren + Shawn + JC.
...
### D8 — Migration: cut over to Graph. There is no legacy corpus to migrate.

> **Rewritten 2026-08-06.** This section previously specified a **dual-anchoring** period, on the premise that a body of existing `ContentHash.Raw` claim anchors had to be preserved and backfilled lazily. That premise was asserted, never checked. It has now been checked against mainnet and it is false. The conclusion — *don't re-mint* — survives; the justification does not, and a migration strategy defending a corpus that does not exist is not worth carrying in the implementation surface.

**Evidence (mainnet, queried 2026-08-06).** The claims engine has produced **eight** Raw anchors on mainnet in total: **4 claims** (2026-03-11 ×2, 03-30, 04-23) and **4 attestation blobs** (2026-03-11, anchored via `MsgAnchor`). All eight return `"raw": {…, "file_extension": "json"}, "graph": null`. Every one is test or demo content — two are explicit fixtures (claimant `orn:personal-koi.entity:claims-engine-test-org`, statements reading *"Test organization restored 50 hectares… (run 1773215949)"*), one is a demo org, one a seed sample about Regen itself. **Zero are third-party ecological claims**, and nothing has been anchored in the ~3.5 months since. Separately, **`MsgAttest` has never been executed** — `AttestationsByAttestor` returns `total: 0` for both signing accounts, so the Graph-only constraint has cost us nothing historically; it is a forward blocker, not a stranded corpus. For contrast, the registry's own data is already all-Graph: 13/13 credit classes, 187/187 projects and 80/80 sampled batches carry `.rdf` IRIs resolving to `graph.canonicalization_algorithm: 1`. Raw anchoring on mainnet is our own dogfooding, not the product.

*Method, so this is re-runnable rather than taken on trust:* `AnchorByIRI` for every claim IRI stored in both claim databases — a current-state lookup, so unaffected by the tx-index pruning that makes March transactions 404 on public nodes — validated first against a **positive control** (a real C01 batch IRI, which returns a full anchor record; without that control a "not found" would mean nothing). Then `AttestationsByAttestor` for both signing accounts. Two independent REST providers agree, and all four claim hashes match the production DB `content_hash` byte for byte.

**Decision:**

- New claims anchor as `ContentHash.Graph` / RDFC-1.0. **No dual-anchor period, and no dual-anchor machinery.**
- The eight existing Raw anchors are re-anchored or abandoned as a one-off. Preserving them as "historical payload-integrity proofs" buys nothing when the preserved bytes read *"Test organization… (run 1773215949)"*.
- Backfill is not *optional and lazy* — it is **unnecessary**. Nothing has ever been attested, so nothing needs a graph fingerprint retroactively.
...
**Bounds on this finding, because they matter.** `x/data` v2 exposes **no list RPC** — all 11 query RPCs are keyed lookups ([`v2/query.proto`](https://github.com/regen-network/regen-ledger/blob/451c3a3f4353fc0d5a82383f2616a284baffa96f/proto/regen/data/v2/query.proto)) — so a chain-wide Raw census cannot be run from a query node. The eight are exhaustive for *our* signing keys across both claim databases; a third `claims-service` keyring elsewhere would be invisible to this method. Nothing suggests one exists. **UNVERIFIED:** the production account reports `sequence: 9` against 8 accounted-for anchors, so one signed transaction is unexplained — most plausibly a failed broadcast, and not re-readable because the public tx index is pruned past block 27420001.

The practical effect on this ADR: the migration burden that formed part of D2-a's cost largely disappears.
...
(3) **D8 rewritten from dual anchoring to a straight cutover, and its algorithm-column bullet promoted to D9.** A mainnet query found **8** Raw anchors from the claims engine, all test or demo content, and **zero** executed `MsgAttest`. The dual-anchor design defended a corpus that does not exist. Conclusion unchanged; justification replaced with evidence; machinery removed.
