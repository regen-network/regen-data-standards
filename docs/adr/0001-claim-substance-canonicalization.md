...
- **Deciders:** Darren Zal (KOI claims engine), Shawn Anderson, Jeancarlo / JC (ybird-labs, RDF claims engine), Marie Gauthier (data standards)
...
2. **Does JC's engine already produce RDFC-1.0 canonical N-Quads over an equivalent substance set?** **Answered 2026-09-08 by reading the repository, and the two halves come apart: RDFC-1.0 yes, equivalent substance set no.**
   - **Algorithm: yes.** `main` (`8cbf0e5d`) names its suite `ClaimValueRdfc10CanonicalNQuadsUtf8Sha256V1`; the newer design branch uses the string `claims-spike/claim-value/rdfc10-nquads-utf8/sha256/v1`. RDFC-1.0 canonical N-Quads, UTF-8, SHA-256 — arrived at independently of D2-a.
   - **Substance: no.** On `main` the preimage is **Claim IRI + canonical content + assertor IRI + `asserted_at`** (`crates/claims/src/domain/fingerprint.rs`). In the 2 July design (`217fafdd`, branch `claude/claims-engine-domain-model-8odk0u`, unmerged) it is **declared schema references + canonical content**, with the ClaimIRI *derived from* the fingerprint rather than an input to it, and assertion provenance removed from the claim value entirely (design doc §19).
   - **That second model is declared and prototyped, not implemented — which matters for what we ask of JC.** The `crates/` tree is byte-identical at both commits (`git rev-parse 8cbf0e5d:crates` and `217fafdd:crates` both give `1d556cc5f16d…`; `git diff -- crates/` is empty), so `AssertionProvenance` and the four-part fingerprint are still present on the branch. The new model lives in the 553-line design doc, in `AGENTS.md` and `README.md`, and in `spike/` — a separate cargo package (`claims-spike`) that the root workspace excludes (`members = ["crates/claims"]`).
   - **The resulting gap is internal to his repo.** The branch's own `AGENTS.md` states *"Do not reintroduce `Assertion`, `AssertionProvenance`, `AssertedAt`, or `AssertorIri` as domain concepts"*, while `crates/claims` on that same branch still defines all four. So the open item is not "which branch is current" but what JC intends for the crate.
   - **Consequence for this ADR:** the adapter is *not* thin, and the residual risk is a silent fork rather than an algorithmic one — see open question 9.
   - **Caveat:** which branch JC considers current is not established. Asked directly by email 2026-09-08; unanswered at time of writing.
...
9. **Do declared schema references belong in the fingerprint preimage? — @JeancarloBarrios / @blushi.** JC's design doc §19.4 states the preimage is "declared schema references + canonical content (was: IRI + content + assertor + asserted_at)", and `spike/src/identity.rs` implements exactly that: a suite label, then the sorted duplicate-free schema IRIs each on its own `schema <iri>` line, then a separator, then the canonical N-Quads. **This ADR's substance set contains no schema declarations at all.** So two implementations that both correctly perform RDFC-1.0 over the same graph still produce different fingerprints — the divergence is invisible at the algorithm layer and shows up only as claims that fail to match. This is the single most likely source of a silent cross-engine fork and it is not covered anywhere else in this ADR.

   Related and unresolved: in his model the ClaimIRI is *derived from* the fingerprint (§19.3), whereas D-series decisions here treat the IRI as minted separately. If we mint IRIs differently from him, identities diverge for byte-identical claims.
